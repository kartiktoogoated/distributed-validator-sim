      ...
      this.peers.map(async (peer, i) => {
        ...
        try {
          ...
          if (res.data.success) {
            ...
            info(
              `Node ${this.id} replicated to ${peer}: matchIndex=${
                this.matchIndex[i]
              }`
            );
            ...
            for (let N = this.log.length - 1; N > this.commitIndex; N--) {
              const count =
                [this.id, ...this.peers.map((_, j) => j)].filter(
                  (j) => this.matchIndex[j] >= N
                ).length;
              if (
            ...
            warn(
              `Node ${this.id} replication to ${peer} failed, nextIndex now ${
                this.nextIndex[i]
              }`
            );
          }
